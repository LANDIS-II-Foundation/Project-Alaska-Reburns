...
combined_csv<-rbind(baseline_csv1, baseline_csv2, baseline_csv3, baseline_csv4, baseline_csv5, temp_csv1, temp_csv2, temp_csv3, temp_csv4, temp_csv5, SM_csv1, SM_csv2, SM_csv3, SM_csv4, SM_csv5)

scenario_name<-c(rep("Baseline", replicates*nrow(baseline_csv1)), (rep("Soil_temp", replicates*nrow(temp_csv1))), (rep("Soil_moisture", replicates*nrow(SM_csv1))))
...
ggplot(all_data_ribbon, aes(x=(Year), y=mean_AGB, group=scenario_name)) +
  geom_line(aes(x=(Year), y=mean_AGB, colour=scenario_name), size=1.1)+
  geom_ribbon(aes(ymin=mean_AGB-SD_AGB, ymax=mean_AGB+SD_AGB, fill=scenario_name), alpha=0.25, show.legend = FALSE)+ 
  scale_color_manual(name = "Scenario", labels = c("Climate Change", "CC, No soil temp limitation", "CC, No soil moisture limitation"), values = plt.cols.short) +
...
#nitrogen_AGB<-subset (all_data_ribbon, all_data_ribbon$scenario_name=="Nitrogen")
...
all_data_ribbon_SM_ST<-subset (all_data_ribbon, !all_data_ribbon$scenario_name=="Nitrogen")
#plt.cols.short <- c("grey30", "cyan", "darkorange") #Number corresponds to scenarios

myblue <- rgb(70, 130, 180, max = 255, alpha = 160) #125 is midway
mygreen <- rgb(34, 139, 34, max = 255, alpha = 160) #125 is midway
plt.cols.shorter <- c(mygreen, myblue)

Difference_matrix_SM_ST<-subset (Difference_matrix, !Difference_matrix$scenario_name=="Nitrogen")
Difference_matrix_SM_ST$scenario_name <- factor(Difference_matrix_SM_ST$scenario_name, levels = c("Soil_moisture", "Soil_temp"))
...
plot<-ggplot(data = Difference_matrix_SM_ST , aes(x = Time, y = Diffs, fill = scenario_name)) +
...
RI_plot <- ggplot(Difference_matrix_SM_ST, aes(x=(Time), y=Diffs, fill= scenario_name)) +
  geom_bar(position="fill", stat="identity")+
  #geom_ribbon(aes(ymin=AGB_Carbon-AGB_SD_Carbon, ymax=AGB_Carbon+AGB_SD_Carbon, fill=Scenario), alpha=0.25, show.legend = FALSE)+ 
  scale_fill_manual(name = "Limiting Factor", labels = c("Soil temperature", "Soil moisture"), values = plt.cols.shorter) +
  theme_classic()+
  xlab(NULL)+ylab(expression(Relative~importance~of~different~factors~on~aboveground~biomass)) + theme(legend.position = "right")+
  scale_x_continuous(breaks=seq(1990, 2040, 10)) + scale_y_continuous(breaks=seq(0,1,0.25))
RI_plot + theme(legend.position = c(0.7, 0.3)) + #this adjusts the legend location.
...
model_dir <- "C:/Users/mlucash/Dropbox (University of Oregon)/Lucash_Lab/Lucash/Alaska_Reburns_Project/Manuscripts/DGS_model_description_paper/Figures/Rel_ImportGraphs/"  
setwd(model_dir)
...
ggsave("RelativeImport_Trees.TIFF", marrangeGrob(grobs=plots, nrow=1, ncol=2, dpi=400))
dev.off()
...
scenario_name<-c(rep("Baseline", 5*nrow(baseline_csv1)), rep("Soil temp", 5*nrow(baseline_csv1)), (rep("Soil moisture", 5*nrow(SM_limit_csv1))))
combined_files<-cbind(scenario_name, combined_csv)
combined_files$Year<-combined_files$Time + year1
...
all_data_ribbon$scenario_name <- factor(all_data_ribbon$scenario_name, levels = c("Baseline", "Soil temp", "Soil moisture"))
...
ggplot(all_data_ribbon, aes(x=(Year), y=mean_AGB, group=scenario_name)) +
  ...
  scale_fill_manual(values = plt.cols.short)+
  scale_x_continuous(breaks=seq(1990, 2040, 10)) + scale_y_continuous(limits=c(0,8000), breaks=seq(0, 8000, 2000))+
...
#Difference values
baseline_AGB<-subset (combined_files, combined_files$scenario_name=="Baseline")
temp_AGB<-subset (combined_files, combined_files$scenario_name=="Soil temp")
SM_AGB<-subset (combined_files, combined_files$scenario_name=="Soil moisture")
...
Time<-rep(baseline_AGB$Year, 2) # 2= SM+ST
...
Difference_matrix$scenario_name <- factor(Difference_matrix$scenario_name, levels = c("Soil temp", "Soil moisture"))

myblue <- rgb(70, 130, 180, max = 255, alpha = 130) #125 is midway
mygreen <- rgb(34, 139, 34, max = 255, alpha = 150) #125 is midway
plt.cols.shorter <- c(mygreen, myblue) #Number corresponds to scenarios
...
ggplot(Difference_matrix, aes(x=(Time), y=Diffs, fill= scenario_name)) +
  ...
  scale_fill_manual(name = "Limiting Factor", labels = c("Soil temperature", "Soil moisture"), values = plt.cols.shorter) +
  theme_classic()+
  xlab(NULL)+ylab(expression(Relative~importance~of~different~factors~on~regeneration)) + theme(legend.position = "right")+
  scale_x_continuous(breaks=seq(1990, 2040, 10)) + scale_y_continuous(breaks=seq(0,1,0.25))+
theme(legend.position = c(0.7, 0.3)) + #this adjusts the legend location.
  theme(axis.title.y = element_text(vjust = 3)) #this adjusts the axis title location
dev.off()
